...
plotTfHeatmapClust2 <- function(yhat, 
  ...
  # dfAnnRow$set[dfAnnRow$clus == 3] <- "Cell cycle, P53 signal transduction"
  # dfAnnRow$set[dfAnnRow$clus == 4] <- "Cell cycle, DNA recombination, chromatin"
  dfAnnRow$set[dfAnnRow$clus == 1] <- "Stress"
  dfAnnRow$set[dfAnnRow$clus == 2] <- "Dendrite proj."
  dfAnnRow$set[dfAnnRow$clus == 3] <- "Cell cycle"
  dfAnnRow$set[dfAnnRow$clus == 4] <- "Neuron diff."
  dfAnnRow <- dfAnnRow[,"set", drop=FALSE]
...
cowplot::plot_grid(pHeatClust[[4]], pConnect, nrow=1, labels=letters[1:2])
ggsave("../plots/peakAnalysis_Thresholded/clusteredHeatmapConnectivity.pdf",
       width=10, height=6)
ggsave("../plots/peakAnalysis_Thresholded/clusteredHeatmapConnectivity.png",
       width=10, height=6)
```

...
clHBCMerged <- as.character(getCellColData(archProj, "clHBCMerged")$clHBCMerged)
clHBCMerged[clHBCMerged == "C1_Inj"] <- "Activated"
clHBCMerged[clHBCMerged == "C2_Hybrid"] <- "Hybrid"
clHBCMerged[clHBCMerged == "C3_UI"] <- "Resting"
archrUmap$clHbcMerged <- factor(clHBCMerged)
...
```{r, eval=FALSE}
bothModalities <- list(rna, atac)
for (i in 1:length(bothModalities)) {
    bothModalities[[i]] <- SCTransform(bothModalities[[i]], verbose = FALSE)
}
integrationFeatures <- SelectIntegrationFeatures(object.list = bothModalities, nfeatures = 3000)
bothModalities <- PrepSCTIntegration(object.list = bothModalities, anchor.features = integrationFeatures, verbose = FALSE)
integrationAnchors <- FindIntegrationAnchors(object.list = bothModalities, 
                                             normalization.method = "SCT", 
                                             anchor.features = pancreas.features, 
                                             verbose = FALSE)
integrated <- IntegrateData(anchorset = integrationAnchors, dims = 1:30)
...
We expect Fox transcription factors to be higher expressed in hybrid clusters.
...
# scATAC-seq for slides v1
p1 <- DimPlot(atac, reduction = "umap", group.by="treatment", label=FALSE, cols=c("steelblue", "orange")) + NoLegend() + ggtitle("ATAC treatment") + xlim(c(-6,6))
clPlot <- atac$clHBCMerged
clPlot[clPlot == "C1_inj"] <- "Activated"
clPlot[clPlot == "C2_hybrid"] <- "Hybrid"
clPlot[clPlot == "C3_UI"] <- "Resting"
atac$clPlot <- clPlot
p3 <- DimPlot(atac, group.by = "clPlot", label = FALSE, repel = TRUE, label.size=6) + ggtitle("scATAC-seq clusters") + NoLegend() + scale_colour_hue(drop = FALSE) + xlim(c(-6,6))
p1 + p3
ggsave("~/tmp/scATAC_dr.png", height=7, width=8.7, dpi=500)

# scATAC-seq for slides v2
p1 <- DimPlot(atac, reduction = "umap", group.by="treatment", label=FALSE, cols=c("steelblue", "orange")) + NoLegend() + ggtitle("ATAC treatment") + xlim(c(-6,6))
p3 <- DimPlot(atac, group.by = "predAnn", label = FALSE, repel = TRUE, label.size=6) + ggtitle("scATAC-seq clusters") + NoLegend() + scale_colour_hue(drop = FALSE) + xlim(c(-6,6))
p1 + p3
ggsave("~/tmp/scATAC_dr_v2.png", height=7, width=8.7, dpi=500)

# scRNA-seq for slides
pRNA <- DimPlot(rna, group.by = "celltype", label = TRUE, repel = TRUE, label.size=6) + NoLegend() + ggtitle("scRNA-seq")
# 3 plots with markers for each of C1-C3
cowplot::plot_grid(plotlist=list(pRNA,pc1,pc2,pc3), nrow=1)
ggsave("~/tmp/scRNA_HBC.png", height=7, width=14, dpi=500)


# cell label transfer
p1 <- DimPlot(atac, reduction = "umap", group.by="treatment", label=TRUE, label.size=6) + NoLegend() + ggtitle("scATAC-seq treatment") + xlim(c(-6,6))
p2 <- DimPlot(atac, group.by = "predictedCtHybrid", label = TRUE, repel = TRUE, label.size=6) + ggtitle("scATAC-seq predicted state") +
    NoLegend() + scale_colour_hue(drop = FALSE)  + xlim(c(-6,6))
p3 <- DimPlot(atac, group.by = "clPlot", label = TRUE, repel = TRUE, label.size=6) + ggtitle("scATAC-seq clusters") + 
    NoLegend() + scale_colour_hue(drop = FALSE)  + xlim(c(-6,6))
p4 <- DimPlot(rna, group.by = "ctHybrid", label = TRUE, repel = TRUE, label.size=6) + ggtitle("scRNA-seq") + 
    NoLegend()
p1 + p2 + p3 + p4
ggsave("~/tmp/scATAC_labelTransfer.png", height=7, width=8.7, dpi=500)


# cell label transfer v2
p1 <- DimPlot(atac, reduction = "umap", group.by="treatment", label=FALSE, cols=c("steelblue", "orange")) + NoLegend() + ggtitle("ATAC treatment") + xlim(c(-6,6))
p2 <- DimPlot(atac, group.by = "predictedCtHybrid", label = FALSE, repel = TRUE) + ggtitle("ATAC predicted") + 
    NoLegend() + scale_colour_hue(drop = FALSE) + xlim(c(-6,6))
p4 <- DimPlot(rna, group.by = "ctHybridSub", label = FALSE, repel = TRUE) + ggtitle("scRNA-seq clusters") + 
    NoLegend()
p1 + p2 + p4
ggsave("~/tmp/scATAC_labelTransfer_v2.png", height=7, width=15, dpi=450)

# Fox involvement
fox1 <- FeaturePlot(rna, features = "foxSumSelected_scaled") + ggtitle("Enriched FOX TF expression")
df <- data.frame(cl=rna$ctHybridSub,
                 foxScaled=foxSumScaledSelected)
fox2 <- ggplot(df, aes(x=cl, y=foxScaled)) +
  geom_boxplot() +
  theme_classic() +
  ylab("FOX TF scaled expression")
cowplot::plot_grid(fox1, fox2, nrow=1)
ggsave("~/tmp/foxExpression.png", height=7, width=10, dpi=450)

...
## Muc5b
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Muc5b",], g=3), alpha=.3,
       box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

##Sult1c1
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Sult1c1",], g=3), alpha=.3,
       box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Irx5",], g=3), alpha=.6,
       box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)
```
...
customAlpha <- function(y){
  alpha1 <- pmin(as.numeric(factor(colby(y, g=3)))-.8, 1)
  return(alpha1)
}

## Pax3
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Pax3",], g=3),
       alpha=customAlpha(counts["Pax3",]), box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

## Pax7
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Pax7",], g=3), 
       alpha=customAlpha(counts["Pax7",]),box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

## Pax9
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Pax9",], g=3), 
       alpha=customAlpha(counts["Pax9",]), box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

## Dmrt2
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["Dmrt2",], g=3), alpha=.3,
       box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)

## 2610016A17Rik
plot3d(reducedDim(sds), aspect = 'iso', col=colby(counts["2610016A17Rik",], g=3), alpha=.6,
       box=FALSE)
plot3d.SlingshotDataSet(sds, add=TRUE, col=c("black"), lwd=4)
```
...
# Check independent expression of relevant genes

```{r}
hbcActId <- clDatta == "HBC*"
plot(x=counts["Sult1c1",hbcActId],
     y=counts["Muc5b",hbcActId])
```
